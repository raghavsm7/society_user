...
class ImageUtils {
  ...
  Widget showCacheNetworkImage(
    ...
    return CachedNetworkImage(
        ...
        errorWidget: (context, url, error) {
          LogUtils.printLog(tag: "CachedNetworkImage ERROR on $imageURL", message: "$error");
  ...
  Future<File> imgFromCamera() async {
    File image0 = File("");
    await picker.pickImage(source: ImageSource.camera,imageQuality: 50).then((xFile) async {
      if (xFile != null) {
  ...
  Future<File> imgFromGallery() async {
    File image0 = File("");
    await picker.pickImage(source: ImageSource.gallery,imageQuality: 50).then((xFile) async {
      if (xFile != null) {
        File image = File(xFile.path);
        image0 = await _cropImage(image);
      }
  ...
  Widget showSVGIcons(
    ...
    return Padding(
      padding: EdgeInsets.all(
        AppUtils.appUtilsInstance.getPercentageSize(ofWidth: false, percentage: padding),
      ),
  ...
  Widget showSVGImage(
    ...
    return Padding(
      ...
      child: SvgPicture.asset(
        image,
        fit: fit,
        height: AppUtils.appUtilsInstance.getPercentageSize(percentage: height),
        width: AppUtils.appUtilsInstance.getPercentageSize(percentage: (ofWidth) ? width : height),
